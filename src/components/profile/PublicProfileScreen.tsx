...
const PublicProfileScreen: React.FC<PublicProfileScreenProps> = ({ currentUser, viewingProfileId, onNavigate, getGymNameById, onBack }) => {
  ...
  if (profileError) { return <div className="min-h-screen flex flex-col items-center justify-center p-4 text-center text-red-500"><AlertTriangle size={24} className="mb-2"/>{profileError}<button onClick={onBack} className="mt-4 text-sm text-accent-blue underline">Go Back</button></div>; }
  if (!profileData) { return <div className="min-h-screen flex flex-col items-center justify-center p-4 text-center text-brand-gray">Profile data could not be loaded.<button onClick={onBack} className="mt-4 text-sm text-accent-blue underline">Go Back</button></div>; }
