import React, { useState, useEffect, useCallback, useRef } from 'react';
    import { ChevronDown, Search, Bell, CheckCircle, Circle, HelpCircle, PlusCircle, Loader2, Route as RouteIcon, MessageSquare, Video as VideoIcon, PencilLine as DrawingIcon, AlertTriangle, MapPin, User as UserIcon } from 'lucide-react';
    import { AppView, ActivityLogEntry, QuickStatsData, NavigationData } from '../../types';
    ...
    const DashboardScreen: React.FC<DashboardScreenProps> = ({
        ...
        const fetchQuickStats = useCallback(async (gymId: string | null) => {
            if (!currentUser || !gymId) {
                setQuickStats(null); setLoadingQuickStats(false); setQuickStatsError(null); return;
            }
            setLoadingQuickStats(true); setQuickStatsError(null);
            try {
                const { data, error: rpcError } = await supabase.rpc('get_user_gym_quick_stats', { user_id_in: currentUser.id, gym_id_in: gymId });
                if (rpcError) { throw new Error(`Failed to load quick stats: ${rpcError.message}`); }
                if (data) { setQuickStats({ sendsThisMonth: data.sendsThisMonth, highestGradeSent: data.highestGradeSent, betaAddedThisMonth: data.betaAddedThisMonth }); }
                else { setQuickStats({ sendsThisMonth: 0, highestGradeSent: null, betaAddedThisMonth: 0 }); }
            } catch (err: any) {
                ...
                setQuickStats(null);
            } finally { setLoadingQuickStats(false); }
        }, [currentUser]);
        ...
        const renderActivityItem = (log: ActivityLogEntry) => {
            ...
            const userNameDisplay = isCurrentUser ? ( <span className="font-medium">{userName}</span> ) : ( <button onClick={() => onNavigate('publicProfile', { profileUserId: log.user_id })} className="font-medium hover:underline text-brand-green"> {userName} </button> );
            ...
            const routeDisplay = (
                <>
                    {log.route_id && log.route_name ? ( <button onClick={() => onNavigate('routeDetail', { routeId: log.route_id })} className={`font-medium hover:underline ${textColorClass}`}> {routeName} ({routeGrade}) </button> ) : ( <span className={`font-medium ${textColorClass}`}> {routeName} ({routeGrade}) </span> )}
            ...
            const followedUserDisplay = followedUserId ? ( <button onClick={() => onNavigate('publicProfile', { profileUserId: followedUserId })} className="font-medium hover:underline text-brand-green"> {isFollowingCurrentUser ? 'you' : followedUserName} </button> ) : ( <span className="font-medium">{followedUserName}</span> );
            ...
            switch (log.activity_type) {
                ...
                case 'follow_user': icon = <UserIcon size={18} className="text-accent-purple" />; text = <>started following {followedUserDisplay}.</>; break;
            }
            return (
                <div key={log.id} className="bg-white p-3 rounded-lg shadow flex items-start space-x-3">
                    ...
                    <div className="flex-grow"> <div className="text-sm">{userNameDisplay} {text}</div> <p className="text-xs text-gray-500 mt-0.5">{timeAgo(log.created_at)}</p> </div>
        ...
        const renderQuickStats = () => {
            if (loadingQuickStats) { return <div className="flex justify-center items-center p-6"> <Loader2 className="animate-spin text-accent-blue mr-2" size={24} /> Loading stats... </div>; }
            if (quickStatsError) { return <div className="p-6 text-center text-red-500"> <AlertTriangle size={20} className="inline mr-1 mb-0.5"/> {quickStatsError} </div>; }
            if (!quickStats) { return ( <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center"> <div className="p-3 bg-accent-blue/10 rounded"> <p className="text-2xl font-bold text-accent-blue">0</p> <p className="text-sm text-brand-gray">Sends This Month</p> </div> <div className="p-3 bg-accent-purple/10 rounded"> <p className="text-2xl font-bold text-accent-purple">N/A</p> <p className="text-sm text-brand-gray">Highest Grade</p> </div> <div className="p-3 bg-accent-yellow/10 rounded"> <p className="text-2xl font-bold text-accent-yellow">0</p> <p className="text-sm text-brand-gray">Beta Added (Month)</p> </div> </div> ); }
            return ( <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center"> <div className="p-3 bg-accent-blue/10 rounded"> <p className="text-2xl font-bold text-accent-blue">{quickStats.sendsThisMonth}</p> <p className="text-sm text-brand-gray">Sends This Month</p> </div> <div className="p-3 bg-accent-purple/10 rounded"> <p className="text-2xl font-bold text-accent-purple">{quickStats.highestGradeSent || 'N/A'}</p> <p className="text-sm text-brand-gray">Highest Grade</p> </div> <div className="p-3 bg-accent-yellow/10 rounded"> <p className="text-2xl font-bold text-accent-yellow">{quickStats.betaAddedThisMonth}</p> <p className="text-sm text-brand-gray">Beta Added (Month)</p> </div> </div> );
        };
            ...
            <div className="min-h-screen bg-gray-50 flex flex-col">
                ...
                <header className="bg-white shadow-sm p-4 sticky top-0 z-10">
                    ...
                    <div className="relative mt-4">
                        <input type="text" value={searchTerm} onChange={handleSearchChange} className="absolute inset-0 opacity-0 pointer-events-none" aria-hidden="true" />
                        <button onClick={handleSearchClick} className="w-full px-4 py-2 pl-10 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent-blue bg-gray-100 text-left flex items-center text-gray-500 hover:border-gray-400"> <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" size={20} /> {searchTerm || `Search routes at ${activeGymName}...`} </button>
                    </div>
                </header>
